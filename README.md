...
1. Explain what a token is used for. To authenticate a user access to the server or parts of the server. 
2. What steps can you take in your web apps to keep your data secure? Private routes that redirect the user if one has no token.
3. Describe how web servers work. A local machine connects to a service provider. The service provider connects them to the internet. The internet is a connection of servers that is connected through cable underneath the ocean.
